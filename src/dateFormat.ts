...
export default (date, mask, config) => {
    ...
    const defaults = {
        ...
        gmt: false,
        masks: {
            default: "yyyy-mm-dd hh:MM:ss TT",
    ...
    const getWeek = (date) => {
        ...
        const weekDiff = (targetThursday.valueOf() - firstThursday.valueOf()) / (86400000 * 7);
    ...
    if (!mask && kindOf(date) === "string" && !/\d/.test(date)) {
        mask = date;
        date = undefined;
    }
    ...
    mask = String(config.masks[mask] || mask || config.masks["default"]);
    ...
    const flags = {
        d: () => d(),
        dd: () => pad(d()),
        ddd: () => config.i18n.dayNames[D()],
        DDD: () => getDayName({
            y: y(),
            m: m(),
            d: d(),
            _: _(),
            dayName: config.i18n.dayNames[D()],
            short: true
        }),
        dddd: () => config.i18n.dayNames[D() + 7],
        DDDD: () => getDayName({
            y: y(),
            m: m(),
            d: d(),
            _: _(),
            dayName: config.i18n.dayNames[D() + 7]
        }),
        m: () => m() + 1,
        mm: () => pad(m() + 1),
        mmm: () => config.i18n.monthNames[m()],
        mmmm: () => config.i18n.monthNames[m() + 12],
        yy: () => String(y()).slice(2),
        yyyy: () => pad(y(), 4),
        h: () => H() % 12 || 12,
        hh: () => pad(H() % 12 || 12),
        ...
        t: () =>
            H() < 12
                ? config.i18n.timeNames[0]
                : config.i18n.timeNames[1],
        tt: () =>
            H() < 12
                ? config.i18n.timeNames[2]
                : config.i18n.timeNames[3],
        T: () =>
            H() < 12
                ? config.i18n.timeNames[4]
                : config.i18n.timeNames[5],
        TT: () =>
            H() < 12
                ? config.i18n.timeNames[6]
                : config.i18n.timeNames[7],
        Z: () =>
            gmt
                ? "GMT"
                : utc
